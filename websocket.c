...
int ws_send(ws_client_t *client, WEBSOCKET_OPCODES_t opcode, char *msg, uint64_t len, bool mask) {
    ...
    if (mask) {
        out[pos] = header.key.part[0];
        pos++;
        out[pos] = header.key.part[1];
        pos++;
        out[pos] = header.key.part[2];
        pos++;
        out[pos] = header.key.part[3];
        pos++;
...
char *ws_read(ws_client_t *client, ws_header_t *header) {
    ...
    if (header->param.bit.FIN == 0) { // if the message isn't done
        if ((header->param.bit.OPCODE == WEBSOCKET_OPCODE_CONT) &&
            ...
            //free(buf);
            return NULL;
        } else if ((header->param.bit.OPCODE == WEBSOCKET_OPCODE_BIN) ||
                (header->param.bit.OPCODE == WEBSOCKET_OPCODE_TEXT)) {
            if (client->len) {
                free(client->contin);
            }
